...
# Requires Docker. Always creates a disposable database with networking disabled.
# Never reads .env, publishes a port, or connects to an existing database.
...
docker exec "$test_container" psql -X -U postgres -d postgres \
  -v ON_ERROR_STOP=1 -f /work/supabase/tests/workspace-invitation-security.sql
